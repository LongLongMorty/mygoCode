...
// Write/Read tools (per MygoCode's reference architecture). This struct exists
// to give the TUI a stable handle for system-prompt building and for the
// `/memory` slash command (list / clear).
type Manager struct {
	projectRoot string
	userMemDir  string // ~/.mygocode/memory/ — user/feedback type memories
	memDir      string // <projectRoot>/.mygocode/memory/ — project/reference type memories
}
