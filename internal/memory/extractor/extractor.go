...
type Deps struct {
	MemoryDir     string                           // <wd>/.mygocode/memory/ — project/reference (trailing sep)
...
// (MygoCode messages have no uuid; cursor is the parent conversation's
...
func (e *Extractor) runExtraction(ctx context.Context, isTrailingRun bool) error {
	...
	// Throttle: default is 1 (run every turn). MygoCode hardcodes 1; trailing runs bypass the throttle
