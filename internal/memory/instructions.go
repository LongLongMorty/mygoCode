...
//  1. User global: ~/.mygocode/MYGOCODE.md, ~/.mygocode/AGENTS.md
//  2. Project: walk from git root down to workDir, picking up MYGOCODE.md
//     and AGENTS.md in each directory (so the file closest to cwd wins)
//  3. workDir/.mygocode/INSTRUCTIONS.md (legacy)
...
func DiscoverInstructions(workDir string) []InstructionSource {
	...
	if home, err := os.UserHomeDir(); err == nil {
		add(&sources, seen, filepath.Join(home, ".mygocode", "MYGOCODE.md"), projectRoot)
		add(&sources, seen, filepath.Join(home, ".mygocode", "AGENTS.md"), projectRoot)
	}
	for _, dir := range projectInstructionDirs(workDir) {
		add(&sources, seen, filepath.Join(dir, "MYGOCODE.md"), projectRoot)
		add(&sources, seen, filepath.Join(dir, "AGENTS.md"), projectRoot)
	}
	add(&sources, seen, filepath.Join(workDir, ".mygocode", "INSTRUCTIONS.md"), projectRoot)
...
// 允许范围：项目目录（projectRoot）及其子目录，以及用户 home 目录下的 .mygocode/。
...
func isIncludeAllowed(absPath, projectRoot string) bool {
	...
	// 用户 home 下的 .mygocode/ 目录也允许（全局指令文件的 include）
	if home, err := os.UserHomeDir(); err == nil {
		mygocodeDir := filepath.Join(home, ".mygocode")
		if strings.HasPrefix(absPath, mygocodeDir+string(filepath.Separator)) {
			return true
		}
	}
	return false
