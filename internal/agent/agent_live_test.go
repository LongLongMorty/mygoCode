...
func TestLiveSkillCreatorOutputPath(t *testing.T) {
	...
	// Use a temp dir to simulate a project with .mygocode/skills/
	workDir := t.TempDir()
	testSkillsDir := filepath.Join(workDir, ".mygocode", "skills")
	...
	if _, err := os.Stat(expectedFile); os.IsNotExist(err) {
		...
		for _, f := range allFiles {
			if strings.Contains(f, "SKILL.md") && !strings.Contains(f, ".mygocode/skills/") {
				t.Errorf("SKILL.md created at WRONG location: %s (should be under .mygocode/skills/)", f)
				wrongLocation = true
			}
		}
		if !wrongLocation {
