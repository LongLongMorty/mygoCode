...
func TestSkillCreatorOutputsToCorrectDirectory(t *testing.T) {
	...
	// agent creates the new skill → verify it lands in .mygocode/skills/, not root

	workDir := t.TempDir()
	skillsDir := filepath.Join(workDir, ".mygocode", "skills")
...
New skills MUST be created under the .mygocode/skills/ directory.
The full path should be .mygocode/skills/<skill-name>/SKILL.md.
`), 0o644)
	...
	client := &dynamicMock{handlers: []func([]conversation.Message) []llm.StreamEvent{
		...
		// Round 2: user describes, agent creates the skill in .mygocode/skills/
		func(msgs []conversation.Message) []llm.StreamEvent {
			last := msgs[len(msgs)-1]
...
`,
	...
	// THE KEY CHECK: new skill was created inside .mygocode/skills/, NOT in project root
...
func TestRealSkillsLoadAndRunSimulation(t *testing.T) {
	...
	if _, err := os.Stat(skillsDir); os.IsNotExist(err) {
		t.Skip("No .mygocode/skills directory")
	}
