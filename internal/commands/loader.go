...
// LoadUserCommands merges file-based commands from two search paths: 1. ~/.mygocode/commands/ (user
// global) 2. $workDir/.mygocode/commands/ (project).
//
// Later sources override earlier ones on name collision.
func LoadUserCommands(workDir string) []*Command {
	var dirs []string
	if home, err := os.UserHomeDir(); err == nil {
		dirs = append(dirs, filepath.Join(home, ".mygocode", "commands"))
	}
	dirs = append(dirs,
		filepath.Join(workDir, ".mygocode", "commands"),
	)
