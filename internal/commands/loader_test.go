...
func TestLoadUserCommandsMergesAndOverrides(t *testing.T) {
	work := t.TempDir()
	mewProj := filepath.Join(work, ".mygocode", "commands")
	mustWrite(t, filepath.Join(mewProj, "shared.md"), `from mygocode`)
	...
	if !strings.Contains(names["shared"], "from mygocode") {
		t.Errorf("expected 'from mygocode'; got %q", names["shared"])
	}
