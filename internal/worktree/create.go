...
// WorktreesDir is where all MygoCode-managed worktrees live: a single directory inside the repo root
// that's already in .gitignore.
func WorktreesDir(repoRoot string) string {
	return filepath.Join(repoRoot, ".mygocode", "worktrees")
}
...
// <repoRoot>/.mygocode/worktrees/, or resumes it if it already exists.
//
