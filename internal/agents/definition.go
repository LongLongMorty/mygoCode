...
// InitialPrompt, OmitMygocodeMd, RequiredMcpServers) are still parsed so user definitions don't lose
...
type AgentDefinition struct {
	...
	// OmitMygocodeMd drops the MYGOCODE.md hierarchy from this agent's user context. Read-only agents
	// (Explore, Plan) save tokens by skipping it.
	OmitMygocodeMd bool `yaml:"omitMygocodeMd"`
...
func (d *AgentDefinition) ToSpec() SubAgentSpec {
	return SubAgentSpec{
		...
		OmitMygocodeMd:       d.OmitMygocodeMd,
		Skills:               d.Skills,
		Memory:               d.Memory,
