...
func (l *AgentLoader) LoadAll() error {
	...
	if home != "" {
		l.loadDir(filepath.Join(home, ".mygocode", "agents"), "user")
	}

	if l.workDir != "" {
		l.loadDir(filepath.Join(l.workDir, ".mygocode", "agents"), "project")
	}
	...
	for _, entry := range entries {
		...
		if err != nil {
			...
			if l.ErrorWriter != nil {
				fmt.Fprintf(l.ErrorWriter, "[mygocode] agent definition skipped — %s\n", msg)
			}
			continue
		}
		def.Source = source
