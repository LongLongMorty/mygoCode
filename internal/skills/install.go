...
func (f *fetcher) listContents(src *SkillSource, subpath string) ([]contentEntry, error) {
	...
	req.Header.Set("User-Agent", "mygocode-install-skill")
...
// is expected to be the user-global skills tier (~/.mygocode/skills/) so
...
// UserSkillsRoot returns ~/.mygocode/skills, creating the parent if needed
...
func UserSkillsRoot() (string, error) {
	...
	root := filepath.Join(home, ".mygocode", "skills")
