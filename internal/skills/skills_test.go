...
func TestLoadMewcodeSkills(t *testing.T) {
	...
	if _, err := os.Stat(skillsDir); os.IsNotExist(err) {
		t.Skip("No .mygocode/skills directory found")
	}
...
func TestLoadSkillsMergesPriority(t *testing.T) {
	work := t.TempDir()
	mygocodeDir := filepath.Join(work, ".mygocode", "skills", "shared")
	if err := os.MkdirAll(mygocodeDir, 0o755); err != nil {
		t.Fatal(err)
	}
	os.WriteFile(filepath.Join(mygocodeDir, "SKILL.md"), []byte(`---
name: shared
description: project skill from .mygocode
---
mygocode body`), 0o644)
	...
	if !strings.Contains(got.PromptBody, "mygocode body") {
		t.Errorf("expected mygocode body; got body=%q", got.PromptBody)
	}
}

func TestLoadSkillsForkMode(t *testing.T) {
	dir := t.TempDir()
	skillDir := filepath.Join(dir, ".mygocode", "skills", "reviewer")
	os.MkdirAll(skillDir, 0o755)
	os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(`---
name: reviewer
...
func TestLoadSkillsContextFork(t *testing.T) {
	dir := t.TempDir()
	skillDir := filepath.Join(dir, ".mygocode", "skills", "forky")
	os.MkdirAll(skillDir, 0o755)
	os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(`---
name: forky
