...
func skillDirPaths(workDir string) []string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		dirs = append(dirs, filepath.Join(home, ".mygocode", "skills"))
	}
	if workDir != "" {
		dirs = append(dirs, filepath.Join(workDir, ".mygocode", "skills"))
	}
	return dirs
}
...
//  2. ~/.mygocode/skills/         (user global)
//  3. $workDir/.mygocode/skills/  (project, highest priority)
//
...
func LoadCatalog(workDir string) *Catalog {
	...
	// Tier 2: user global
	if home, err := os.UserHomeDir(); err == nil {
		loadTierInto(c, filepath.Join(home, ".mygocode", "skills"), "user")
	}

	// Tier 3: project
	loadTierInto(c, filepath.Join(workDir, ".mygocode", "skills"), "project")
...
func LoadSkills(workDir string) *Catalog {
	...
	c.workDir = workDir
	if home, err := os.UserHomeDir(); err == nil {
		loadTierEager(c, filepath.Join(home, ".mygocode", "skills"), "user")
	}
	loadTierEager(c, filepath.Join(workDir, ".mygocode", "skills"), "project")
	return c
}
