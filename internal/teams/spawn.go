...
// mygocode binary in teammate mode for the given team/member. The workdir argument controls where
...
//
// Output format matches the cmd line parsed by cmd/mygocode/main.go's teammate-mode branch.
func BuildTeammateCLI(teamName, memberName, workdir string) (string, error) {
	exe, err := os.Executable()
	if err != nil {
		return "", fmt.Errorf("locate mygocode binary: %w", err)
	}
	if workdir == "" {
		workdir, _ = os.Getwd()
	}
