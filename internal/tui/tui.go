...
import (
	...
	"mygocode/internal/agents"
	"mygocode/internal/commands"
	"mygocode/internal/compact"
	"mygocode/internal/config"
	"mygocode/internal/conversation"
	"mygocode/internal/filehistory"
	"mygocode/internal/history"
	"mygocode/internal/hooks"
	"mygocode/internal/llm"
	"mygocode/internal/mcp"
	"mygocode/internal/memory"
	"mygocode/internal/memory/extractor"
	"mygocode/internal/permissions"
	"mygocode/internal/planfile"
	"mygocode/internal/prompt"
	"mygocode/internal/sandbox"
	"mygocode/internal/session"
	...
	case initSingleProviderMsg:
		...
		ag.Checker = permissions.NewChecker(
			...
			&permissions.RuleEngine{
				LocalPath: filepath.Join(wd, ".mygocode", "permissions.local.yaml"),
			},
...
func scanFiles(prefix string, limit int) []string {
	...
	skipDirs := map[string]bool{
		...
		"__pycache__": true, ".mygocode": true, "vendor": true,
	}
...
func (m Model) renderPlanApprovalDialog() string {
	...
	header := lipgloss.NewStyle().Foreground(brandPurple).Bold(true).Render(
		" MygoCode has written up a plan and is ready to execute. Would you like to proceed?",
	)
...
func (m Model) handleAgentEvent(ev agent.AgentEvent) (tea.Model, tea.Cmd) {
	...
	case agent.ToolResultEvent:
		...
		if emitted != nil {
			// Flush pending assistant text first so order matches MygoCode's
...
func (m Model) renderBanner() string {
	cat := bannerStyle.Render(` /\_/\    `) + bannerDimStyle.Render("MygoCode v0.1.0") + "\n" +
